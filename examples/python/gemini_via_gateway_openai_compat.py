...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Call Gemini model through local LLM Gateway (OpenAI-compatible API)."
    )
    parser.add_argument(
        "--base-url",
    ...
    parser.add_argument(
        "--model",
        default=os.getenv("DEMO_MODEL", "gemini-2.5-flash"),
        help="Model id from gateway catalog.",
    )
    parser.add_argument(
        "--prompt",
        default="Расскажи о себе. Очень коротко",
        help="User prompt.",
    )
